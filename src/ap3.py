"""
original repo: https://github.com/mfouesneau/asciiplot
...
class ACanvas(object):
    ...
    def extent(self,
               margin_factor: Number = None) -> Tuple[Number,
                                                      Number,
                                                      Number,
                                                      Number]:
    ...
    def _clip_line(self, line_pt_1: Sequence,
                   line_pt_2: Sequence) -> Tuple[Sequence, Sequence]:
        ...
        result = [(pt_1 + t * (pt_2 - pt_1)) for t in (ts[2], ts[3])
...
class AFigure(object):
    ...
    def xlim(self, vmin: Number = None,
             vmax: Number = None) -> Tuple[Number, Number]:
        return self.canvas.xlim(vmin, vmax)

    def ylim(self, vmin: Number = None,
             vmax: Number = None) -> Tuple[Number, Number]:
    ...
    def _draw_axes(self):
        zero_x = self.get_coord(
            0, self.canvas.min_x, self.canvas.x_step, limits=[
                1, self.canvas.x_size])
        ...
        self.output_buffer[zero_x].append('∧')

        zero_y = self.get_coord(
            0, self.canvas.min_y, self.canvas.y_step, limits=[
                1, self.canvas.y_size])
    ...
    def _plot_labels(self):
        ...
        x_zero_coord = self.get_coord(
            0, self.canvas.min_x, self.canvas.x_step, limits=[
                0, self.canvas.x_size])
        y_zero_coord = self.get_coord(
            0, self.canvas.min_y, self.canvas.y_step, limits=[
                1, self.canvas.y_size])
        ...
        if (self.canvas.x_str() is not None):
            ...
            for i, c in enumerate(max_x_str):
                self.output_buffer[max_x_coord + i -
                                   len(max_x_str)][y_zero_coord - 1] = c
    ...
    def _plot_data_with_slope(self, data: AData) -> None:
        ...
        for i, (xi, yi) in e_xy:
            ...
            if not line & self.canvas.coords_inside_data(xi, yi):
                ...
                x_coord = self.get_coord(
                    xi, self.canvas.min_x, self.canvas.x_step)
                y_coord = self.get_coord(
                    yi, self.canvas.min_y, self.canvas.y_step)

                if self.canvas.coords_inside_buffer(x_coord, y_coord):
                    y0_coord = self.get_coord(
                        0, self.canvas.min_y, self.canvas.y_step)
                    if self.draw_axes:
        ...
        if data.plot_slope:
            ...
            for x, y in zip(data.x, data.y):
                if self.canvas.coords_inside_data(x, y):
                    x_coord = self.get_coord(
                        x, self.canvas.min_x, self.canvas.x_step)
                    y_coord = self.get_coord(
                        y, self.canvas.min_y, self.canvas.y_step)
    ...
    def draw(self) -> str:
        self.output_buffer = [
            [" "] *
            self.canvas.y_size for i in range(
                self.canvas.x_size)]
        if self.draw_axes:
            self._draw_axes()
        ...
        trans_result = self.add_axes_labels(
            self.x_axis_label, self.y_axis_label, trans_result)
...
def steppify(x, y):
    ...
    xx[0::2], xx[1::2] = dx, dx
    yy[0::2], yy[1::2] = y, y
    xx = np.concatenate(([x[0] - (dx[0] - x[0])], xx,
                        [x[-1] + (x[-1] - dx[-1])]))
    return xx, yy
...
def hist(
    ...
    n, b = histogram(x, bins=bins, range=xlim,
...
def hist2d(
    ...
    im, ex, ey = np.histogram2d(
        x, y, bins, range=None, normed=normed, weights=weights)
...
def imshow(im, extent=None, width=50, ncolors=16):
    ...
    if not _clr:
        for h in range(
                height):  # first go through the height,  otherwise will roate
